...
function FinanceCard({ r, canUpdate, canDelete, onEdit, onDelete, formatAmount, formatDate }) {
...
export default function FinancePage() {
  ...
  // Redirect plain Members to their personal giving page
  useEffect(() => {
    if (user?.roleName === 'Member') navigate('/finance/my-giving', { replace: true });
  }, [user, navigate]);

  // Granular permission flags — each button type checks its own permission
  ...
  // useRef keeps the timeout ID stable across renders — the old `let` was re-set to null every render
  ...
  // Summary passes the same filters as records so the cards always match what's shown
  ...
  const resetForm = () => {
    setForm({ member_id:'', member_label:'', category_id:'', amount:'', payment_method:'cash', transaction_date:new Date().toISOString().slice(0,10), receipt_number:'', notes:'' });
    setMemberSearch('');
    setMemberResults([]);
    setEditRecord(null);
    setFormError('');
  ...
  const openEdit = (r) => {
    ...
    setMemberResults([]); // clear any stale dropdown from previous search
    setShowForm(true);
      ...
      {isMobile ? (
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {loading ? (
            ...
            <FinanceCard key={r.id} r={r} canUpdate={canUpdate} canDelete={canDelete} onEdit={openEdit} onDelete={handleDelete} formatAmount={formatAmount} formatDate={formatDate} />
          ))}
        </div>
      ) : (
          ...
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse' }}>
              <thead style={{ background:'#f8fafc' }}>
                <tr>
                  {['Member','Category','Amount','Method','Date','Receipt','Recorded By', ...(showActionsCol ? ['Actions'] : [])].map(h => (
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={showActionsCol ? 8 : 7} style={{ padding:'48px', textAlign:'center', color:'#94a3b8' }}>Loading...</td></tr>
                ) : records.length === 0 ? (
                  <tr><td colSpan={showActionsCol ? 8 : 7} style={{ padding:'48px', textAlign:'center', color:'#94a3b8' }}>No records found.</td></tr>
                ) : records.map((r, i) => {
                  const pmStyle = PAYMENT_STYLE[r.payment_method] || PAYMENT_STYLE.cash;
                  return (
