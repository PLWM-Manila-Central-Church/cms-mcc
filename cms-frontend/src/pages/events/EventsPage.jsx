...
export default function EventsPage() {
  ...
  const resetForm = () => {
    setForm({ category_id: '', title: '', description: '', start_date: '',
      end_date: '', start_time: '', location: '', capacity: '',
      registration_deadline: '', status: 'draft' });
            ...
            <div style={s.formRow}>
              ...
              <div style={s.field}>
                ...
                <input type="datetime-local" value={form.registration_deadline}
                  onChange={e => setForm(f => ({ ...f, registration_deadline: e.target.value }))} style={s.input} />
              </div>
            </div>
            ...
            return (
              <div key={ev.id} style={s.card}>
                <div style={s.cardTop}>
                  ...
                  <span style={s.category}>{ev.category?.name}</span>
                </div>
                <div style={s.cardTitle}>{ev.title}</div>
                {ev.description && (
                  <div style={s.cardDesc}>{ev.description.length > 100 ? ev.description.slice(0, 100) + '…' : ev.description}</div>
                )}
                ...
                {ev.registration_deadline && (
                  <div style={{ ...s.cardMeta, color: new Date() > new Date(ev.registration_deadline) ? '#dc2626' : '#64748b' }}>
                    ⏰ Deadline: {new Date(ev.registration_deadline).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}
                  </div>
                )}
                ...
                <div style={s.cardActions}>
                  ...
                  {canCreate && (
                    <>
                      ...
                      {nextStatuses.map(ns => (
                        <button key={ns} onClick={() => handleStatusChange(ev.id, ns)}
                      ...
                      {ev.status === 'draft' && (
                        <button onClick={() => handleDelete(ev.id)} style={s.deleteBtn}>Delete</button>
                      )}
                    </>
                  )}
                </div>
